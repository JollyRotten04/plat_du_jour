...
import LoadingIcon from '../../components/LoadingIcon/LoadingIcon'; // ✅ Import your loading component
...
export default function IngredientsInFridge() {
  ...
  const [loading, setLoading] = useState(false); // ✅ controls LoadingIcon
    ...
    try {
      ...
      if (data.success) {
        ...
        const sortedRecipes = data.data.sort((a, b) => {
          const aIngredients = a.recipe_ingredients.toLowerCase().split(',').map((x) => x.trim());
          const bIngredients = b.recipe_ingredients.toLowerCase().split(',').map((x) => x.trim());

          const aMatchCount = aIngredients.filter((ing) => selectedLower.includes(ing)).length;
          const bMatchCount = bIngredients.filter((ing) => selectedLower.includes(ing)).length;
  ...
  const viewContent = (
    ...
    console.log('Data from IngredientsInFridge:', data);
                  ...
                  {!selectedCategory &&
                    categories.map((category) => (
                      <div
                        ...
                        className="flex cursor-pointer flex-col items-center gap-3 p-6 sm:p-4 md:p-8 rounded-xl transition-transform duration-150 ease-out transform border-4 border-transparent hover:bg-gray-200 shadow-sm"
                      >
                    ...
                    currentCategory?.items.map((item) => {
                      ...
                      return (
                        <div
                          ...
                          className={`flex cursor-pointer flex-col items-center gap-3 p-6 sm:p-4 md:p-8 rounded-xl transition-transform duration-150 ease-out transform border-4 ${
                            isSelected
