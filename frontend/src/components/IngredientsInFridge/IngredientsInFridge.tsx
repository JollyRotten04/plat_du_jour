    ...
    >
      ...
      <div className="flex flex-col p-4 sm:p-8 gap-4 bg-white h-full w-full md:landscape:w-1/2 xl:landscape:w-2/5 rounded-2xl overflow-y-auto overflow-x-hidden">
        ...
        <div className="flex flex-col mb-4">
          <h2 className="text-lg sm:text-xl font-semibold">What's in your fridge</h2>
          <p className="text-[0.8rem] sm:text-base mb-2 text-gray-600">
       ...
       <div className="grid grid-cols-1 min-[360px]:grid-cols-2 gap-4 sm:gap-6">
          {categories.map((category) => (
            <div
              key={category.name}
              className="flex flex-col items-center gap-3 p-6 sm:p-4 md:p-8 bg-gray-100 rounded-xl shadow-sm hover:bg-gray-200 transition-colors"
            >
        ...
        <div className='flex flex-col items-center mt-8 gap-4'>
          ...
          <p className='text-[0.8rem] sm:text-lg mb-2 text-gray-600'>Items Selected:</p>

          {/* Suggest Button */}
          <button className='px-4 py-2 sm:px-5 sm:py-3 main-background text-white sm:text-xl font-semibold rounded-lg'>Suggest</button>
        </div>
      </div>
    </div>
  );
